...
- (IBAction)mailButtonClick:(UIButton *)sender {
    ...
    if(self.selectedArticleUrl.length != 0) {
        if(mailHeaderString.length != 0) {
            mailBodyStr = [NSString stringWithFormat:@"%@\n\n%@\n\n%@\n\n%@",mailHeaderString,self.selectedArticleTitle,self.articleDesc,self.selectedArticleUrl];
        } else {
        ...
        if(mailHeaderString.length != 0) {
            mailBodyStr = [NSString stringWithFormat:@"%@\n\n%@\n\n%@\n",mailHeaderString,self.selectedArticleTitle,self.articleDesc];
        } else {
...
-(void)loadCuratedNewsDetails:(id)sender {
   ...
   // NSLog(@"Curated News ---> %@",fetchRequest);
    ...
    if(newPerson.count != 0) {
        ...
        NSLog(@"Curated Details:%@",curatedNewsDetail);
        NSSet *relatedPostSet = [curatedNewsDetail valueForKey:@"relatedPost"];
        NSLog(@"related POst Array--->%@",relatedPostSet);
        
        NSMutableArray *postArray = [[NSMutableArray alloc]initWithArray:[relatedPostSet allObjects]];
        NSLog(@"After --->%@",postArray);
        
        self.relatedPostArray = [[NSMutableArray alloc]initWithArray:postArray];
