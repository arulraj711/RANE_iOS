...
- (NSString *)formattedAsTimeAgo
{
    ...
    NSDate *myNewDate = [dateFormatter dateFromString:currentTime];
    NSLog(@"now is----%@",myNewDate);

    NSDateComponents *components= [[NSDateComponents alloc] init];
    [components setHour:5];
    [components setMinute:30];
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSDate *now=[calendar dateByAddingComponents:components toDate:myNewDate options:0];
    NSLog(@"myNewDate is----%@",now);
