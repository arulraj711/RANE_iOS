...
//    NSString *stringWithoutSpaces = [headerColor stringByReplacingOccurrencesOfString:@"#" withString:@""];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loadArticleListFromAPI:) name:@"CuratedNews" object:nil];
...
-(void)notifyForUnread{
    ...
    _spinner.hidden=NO;
    switchForFilter = 1;
    [self callSearchAPIWithStringForUnread:searchBar.text withFilterString:@"UNREAD"];
    _spinner.hidden=YES;
...
-(void)notifyForLast{
    ...
    _spinner.hidden=NO;
    switchForFilter = 2;
    [self callSearchAPIWithStringForUnread:searchBar.text withFilterString:@"RECENT"];
    _spinner.hidden=YES;
...
-(void)notifyForMarkAsRead {
    ...
    if(switchForFilter == 1){
        ...
        if(self.segmentControl.selectedSegmentIndex == 1) {
            ...
            //No need to handle none articles
            isUnRead = NO;
        } else {
            NSArray *unreadArticleArray = [self getUnreadArticlesCount];
            if(unreadArticleArray.count != 0){
                isUnRead = YES;
            } else {
                isUnRead = NO;
            }
    ...
    if([[FISharedResources sharedResourceManager]serviceIsReachable] && isUnRead) {
        [[FISharedResources sharedResourceManager]setUserActivitiesOnArticlesWithDetails:resultStr withFlag:YES];
    } else {
        NSLog(@"all are read articles");
        [self.view makeToast:@"Please select unread article" duration:0.5 position:CSToastPositionCenter];
    }
...
-(NSArray *)getUnreadArticlesCount {
    ...
    if(isDailyDigestClick) {
        ...
        if([categoryId isEqualToNumber:[NSNumber numberWithInt:-3]]) {
            if(isSearching) {
                ...
                predicate  = [NSPredicate predicateWithFormat:@"saveForLater == %@ AND categoryId==%@ AND isFilter == %@ AND readStatus==%@",[NSNumber numberWithBool:YES],categoryId,[NSNumber numberWithInt:switchForFilter],[NSNumber numberWithBool:NO]];
            } else if(switchForFilter == 2) {
                predicate  = [NSPredicate predicateWithFormat:@"saveForLater == %@ AND categoryId==%@ AND isFilter == %@ AND readStatus==%@",[NSNumber numberWithBool:YES],categoryId,[NSNumber numberWithInt:switchForFilter],[NSNumber numberWithBool:NO]];
            } else {
            ...
            if(isSearching) {
                predicate  = [NSPredicate predicateWithFormat:@"categoryId==%@ AND contentTypeId==%@ AND isSearch == %@ AND readStatus==%@",categoryId,contentTypeId,[NSNumber numberWithBool:isSearching],[NSNumber numberWithBool:NO]];
            } else if(switchForFilter == 1) {
                predicate  = [NSPredicate predicateWithFormat:@"categoryId==%@ AND contentTypeId==%@ AND isFilter == %@ AND readStatus==%@",categoryId,contentTypeId,[NSNumber numberWithInt:switchForFilter],[NSNumber numberWithBool:NO]];
            } else if(switchForFilter == 2) {
                predicate  = [NSPredicate predicateWithFormat:@"categoryId==%@ AND contentTypeId==%@ AND isFilter == %@ AND readStatus==%@",categoryId,contentTypeId,[NSNumber numberWithInt:switchForFilter],[NSNumber numberWithBool:NO]];
            } else {
        ...
        if(isSearching) {
            predicate  = [NSPredicate predicateWithFormat:@"isFolder == %@ AND folderId == %@ AND isSearch == %@ AND readStatus==%@",[NSNumber numberWithBool:YES],folderId,[NSNumber numberWithBool:isSearching],[NSNumber numberWithBool:NO]];
        } else if(switchForFilter == 1) {
            predicate  = [NSPredicate predicateWithFormat:@"isFolder == %@ AND folderId == %@ AND isFilter == %@ AND readStatus==%@",[NSNumber numberWithBool:YES],folderId,[NSNumber numberWithInt:switchForFilter],[NSNumber numberWithBool:NO]];
        } else if(switchForFilter == 2) {
            predicate  = [NSPredicate predicateWithFormat:@"isFolder == %@ AND folderId == %@ AND isFilter == %@ AND readStatus==%@",[NSNumber numberWithBool:YES],folderId,[NSNumber numberWithInt:switchForFilter],[NSNumber numberWithBool:NO]];
        } else {
    ...
    [fetchRequest setPredicate:predicate];
    NSArray *unreadArticle =[[managedObjectContext executeFetchRequest:fetchRequest error:nil] mutableCopy];
    
    NSLog(@"curated news list count:%lu",(unsigned long)unreadArticle.count);
...
- (void)callSearchAPIWithStringForUnread:(NSString *)searchString withFilterString:(NSString *)filterString {
    ...
    if(isFromChart) {
        ...
        if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:1]]) {
            //Trend of coverage
            [[FISharedResources sharedResourceManager]getTrendOfCoverageArticleListFromDate:self.clickedDate endDateIn:self.trendOfCoverageEndDateIn fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInt:0] withFilterBy:filterString withQuery:searchString withFlag:@"" withLastArticleId:@""];
        } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:2]]) {
            //Key topics
            [[FISharedResources sharedResourceManager]getKeyTopicsArticleListFromField1:@"fields.name" value1:self.keyTopicsBrandName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInt:0] withFilterBy:filterString withQuery:searchString withFlag:@"" withLastArticleId:@""];
        } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:3]]) {
            //Media types
            [[FISharedResources sharedResourceManager]getMediaTypesArticleListFromMediaTypeField:@"mediaTypeId" mediaTypeValue:self.mediaTypesBrandName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInt:0] withFilterBy:filterString withQuery:searchString withFlag:@"" withLastArticleId:@""];
        } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:4]]) {
            //sentiment and volume over time
            [[FISharedResources sharedResourceManager]getSentimentOverTimeArticleListFromDate:self.clickedDate endDateIn:@"MONTH" field1:@"tonality.name" field2:@"fields.name" value1:self.sentimentChartTonalityValue value2:self.sentimentChartSelectedName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInt:0] withFilterBy:filterString withQuery:searchString withFlag:@"" withLastArticleId:@""];
            ...
            //top sources
            [[FISharedResources sharedResourceManager]getHorizontalLineBarChartArticleListFromField1:@"tonality.name" field2:@"outlet.name.sort" value1:self.horizontalBarChartTonalityValue value2:self.horizontalBarChartSelectedValue fromDate:self.reportFromDate toDate:self.reportToDate withSize:[NSNumber numberWithInt:10] withModules:self.reportModules withTags:self.reportTags withPageNo:[NSNumber numberWithInt:0] withFilterBy:filterString withQuery:searchString withFlag:@"" withLastArticleId:@""];
        ...
        if([folderId isEqualToNumber:[NSNumber numberWithInt:0]] && [newsLetterId isEqualToNumber:[NSNumber numberWithInt:0]]) {
            ...
            if([category isEqualToNumber:[NSNumber numberWithInt:-2]]) {
                ...
                queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] valueForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchString withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:filterString withActivityTypeID:[NSNumber numberWithInt:2] withCompanyId:self.companyId];
                ...
                queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchString withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:filterString withActivityTypeID:[NSNumber numberWithInt:3] withCompanyId:self.companyId];
                ...
                queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:parentId withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchString withContentCategoryId:category withOrderBy:@"" withFilterBy:filterString withActivityTypeID:[NSNumber numberWithInt:0] withCompanyId:self.companyId];
...
-(void)loadArticleListFromAPI:(id)sender {
    
    NSNotification *notification = sender;
    NSDictionary *userInfo = notification.userInfo;
    isFromChart = [[userInfo objectForKey:@"isFromChart"] boolValue];
...
-(void)loadCuratedNews {
    ...
    if(isDailyDigestClick) {
        ...
        if(isSearching) {
            predicate = [NSPredicate predicateWithFormat:@"isFromChart == %@ AND isSearch == %@",[NSNumber numberWithBool:YES],[NSNumber numberWithBool:isSearching]];
        } else if(switchForFilter == 1) {
            predicate = [NSPredicate predicateWithFormat:@"isFromChart == %@ AND isFilter == %@",[NSNumber numberWithBool:YES],[NSNumber numberWithInt:switchForFilter]];
        } else if(switchForFilter == 2) {
            predicate = [NSPredicate predicateWithFormat:@"isFromChart == %@ AND isFilter == %@",[NSNumber numberWithBool:YES],[NSNumber numberWithInt:switchForFilter]];
        } else {
...
- (void)refreshTable {
    ...
    if(isDailyDigestClick) {
        ...
        if (switchForFilter == 1){
            [[FISharedResources sharedResourceManager]fetchArticleFromNewsLetterWithAccessToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withNewsLetterId:[NSNumber numberWithInt:0] withPageNo:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withUpFlag:NO withFlag:NO withQuery:searchBar.text withFilterBy:@"UNREAD"];
            
            
        } else if (switchForFilter == 2){
            [[FISharedResources sharedResourceManager]fetchArticleFromNewsLetterWithAccessToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withNewsLetterId:[NSNumber numberWithInt:0] withPageNo:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withUpFlag:NO withFlag:NO withQuery:searchBar.text withFilterBy:@"RECENT"];
            
        }else {
            [[FISharedResources sharedResourceManager]fetchArticleFromNewsLetterWithAccessToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withNewsLetterId:[NSNumber numberWithInt:0] withPageNo:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withUpFlag:NO withFlag:NO withQuery:searchBar.text withFilterBy:@""];
        }
    } else if(isFromChart) {
        ...
        if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:1]]) {
            ...
            //sentiment
            [[FISharedResources sharedResourceManager]getKeyTopicsArticleListFromField1:@"tonality.name" value1:self.keyTopicsBrandName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInt:0] withFilterBy:filterString withQuery:searchBar.text withFlag:@"up" withLastArticleId:@""];
        }
    } else {
        if([folderId isEqualToNumber:[NSNumber numberWithInt:0]] && [newsLetterId isEqualToNumber:[NSNumber numberWithInt:0]]) {
            // NSInteger category = categoryStr.integerValue;
            if([category isEqualToNumber:[NSNumber numberWithInt:-2]]) {
                ...
                //            } else
                if (switchForFilter == 1){
                    queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchBar.text withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:@"UNREAD" withActivityTypeID:[NSNumber numberWithInt:2] withCompanyId:self.companyId];
                    
                } else if (switchForFilter == 2){
                    queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchBar.text withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:@"RECENT" withActivityTypeID:[NSNumber numberWithInt:2] withCompanyId:self.companyId];
                ...
                //            } else
                if (switchForFilter == 1){
                    queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchBar.text withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:@"UNREAD" withActivityTypeID:[NSNumber numberWithInt:3] withCompanyId:self.companyId];
                    
                } else if (switchForFilter == 2){
                    queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchBar.text withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:@"RECENT" withActivityTypeID:[NSNumber numberWithInt:3] withCompanyId:self.companyId];
                ...
                //            } else
                if (switchForFilter == 1){
                    queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchBar.text withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:@"UNREAD" withActivityTypeID:[NSNumber numberWithInt:0] withCompanyId:self.companyId];
                    
                } else if (switchForFilter == 2){
                    queryString = [FIUtils formArticleListInuptFromSecurityToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withContentTypeId:[NSNumber numberWithInt:1] withPageNumber:[NSNumber numberWithInt:0] withSize:[NSNumber numberWithInt:10] withQuery:searchBar.text withContentCategoryId:[NSNumber numberWithInt:-1] withOrderBy:@"" withFilterBy:@"RECENT" withActivityTypeID:[NSNumber numberWithInt:0] withCompanyId:self.companyId];
...
//To enable or disable row swipe in UITableview
...
-(void)savedActions:(NSIndexPath *)tapGesture {
    ...
    if([[FISharedResources sharedResourceManager]serviceIsReachable]) {
        if([[curatedNews valueForKey:@"saveForLater"] isEqualToNumber:[NSNumber numberWithInt:1]]) {
            ...
            [self.view makeToast:brandingName duration:1.0 position:CSToastPositionCenter];
    ...
    if([[FISharedResources sharedResourceManager]serviceIsReachable]) {
        if([number isEqualToNumber:[NSNumber numberWithInt:1]]) {
            ...
            if([markedImpUserId isEqualToString:@"-1"]) {
                ...
                NSString *brandingName = [NSString stringWithFormat:@"%@",[brandingIdentity valueForKey:@"name"]];
                [self.view makeToast:brandingName duration:2.0 position:CSToastPositionCenter];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(self.devices.count != 0) {
        ...
        [cell.articleImageView sd_setImageWithURL:[NSURL URLWithString:[curatedNews valueForKey:@"image"]] placeholderImage:[UIImage imageWithContentsOfFile:path]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (longPressActive) { //Perform action desired when cell is long pressed
        ...
        else
        {
            ...
            [articleIdArray addObject:articleId];
        ...
        if(self.devices.count != 0) {
            ...
            testView.companyIdInDetailPage = self.companyId;
            testView.reportTypeId = self.reportTypeId;
            testView.reportModules = self.reportModules;
            testView.reportTags = self.reportTags;
...
-(void)checkMark:(UITapGestureRecognizer *)tapGesture {
    ...
    NSLog(@"after selection filter array%lu",(unsigned long)self.filterArray.count);
}
    ...
    if(self.devices.count != 0){
        ...
        if(y > h + reload_distance) {
            ...
            if(isDailyDigestClick) {
                ...
                if (switchForFilter == 1){
                    [[FISharedResources sharedResourceManager]fetchArticleFromNewsLetterWithAccessToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withNewsLetterId:[NSNumber numberWithInt:0] withPageNo:[NSNumber numberWithInteger:pageNo] withSize:[NSNumber numberWithInt:10] withUpFlag:NO withFlag:NO withQuery:searchBar.text withFilterBy:@"UNREAD"];
                    
                    
                } else if (switchForFilter == 2){
                    [[FISharedResources sharedResourceManager]fetchArticleFromNewsLetterWithAccessToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withNewsLetterId:[NSNumber numberWithInt:0] withPageNo:[NSNumber numberWithInteger:pageNo] withSize:[NSNumber numberWithInt:10] withUpFlag:NO withFlag:NO withQuery:searchBar.text withFilterBy:@"RECENT"];
                    
                } else {
                    [[FISharedResources sharedResourceManager]fetchArticleFromNewsLetterWithAccessToken:[[NSUserDefaults standardUserDefaults] objectForKey:@"accesstoken"] withNewsLetterId:[NSNumber numberWithInt:0] withPageNo:[NSNumber numberWithInteger:pageNo] withSize:[NSNumber numberWithInt:10] withUpFlag:NO withFlag:NO withQuery:searchBar.text withFilterBy:@""];
                }
            } else if(isFromChart) {
                NSString *filterString;
                if (switchForFilter == 1){
                ...
                if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:1]]) {
                    //Trend of coverage
                    [[FISharedResources sharedResourceManager]getTrendOfCoverageArticleListFromDate:self.clickedDate endDateIn:self.trendOfCoverageEndDateIn fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInteger:10] withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:2]]) {
                    //key topics
                    [[FISharedResources sharedResourceManager]getKeyTopicsArticleListFromField1:@"fields.name" value1:self.keyTopicsBrandName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:3]]) {
                    //Media types
                    [[FISharedResources sharedResourceManager]getMediaTypesArticleListFromMediaTypeField:@"mediaTypeId" mediaTypeValue:self.mediaTypesBrandName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:4]]) {
                    //sentiment and volume over time
                    [[FISharedResources sharedResourceManager]getSentimentOverTimeArticleListFromDate:self.clickedDate endDateIn:@"MONTH" field1:@"tonality.name" field2:@"fields.name" value1:self.sentimentChartTonalityValue value2:self.sentimentChartSelectedName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:5]]) {
                    //change over last quarter value
                    [[FISharedResources sharedResourceManager]getChangeOverLastQuarterArticleListFromDate:self.clickedDate endDateIn:@"MONTH" field1:@"fields.name" value1:self.changeOverSelectedValue fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:6]]) {
                    //top sources
                    [[FISharedResources sharedResourceManager]getHorizontalLineBarChartArticleListFromField1:@"tonality.name" field2:@"outlet.name.sort" value1:self.horizontalBarChartTonalityValue value2:self.horizontalBarChartSelectedValue fromDate:self.reportFromDate toDate:self.reportToDate withSize:[NSNumber numberWithInt:10] withModules:self.reportModules withTags:self.reportTags withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:7]]) {
                    //top journalist
                    [[FISharedResources sharedResourceManager]getHorizontalLineBarChartArticleListFromField1:@"tonality.name" field2:@"contact.name.sort" value1:self.horizontalBarChartTonalityValue value2:self.horizontalBarChartSelectedValue fromDate:self.reportFromDate toDate:self.reportToDate withSize:[NSNumber numberWithInt:10] withModules:self.reportModules withTags:self.reportTags withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                } else if([self.reportTypeId isEqualToNumber:[NSNumber numberWithInt:8]]) {
                    ...
                    //sentiment
                    [[FISharedResources sharedResourceManager]getKeyTopicsArticleListFromField1:@"tonality.name" value1:self.keyTopicsBrandName fromDate:self.reportFromDate toDate:self.reportToDate withModules:self.reportModules withTags:self.reportTags withSize:[NSNumber numberWithInt:10] withPageNo:[NSNumber numberWithInteger:pageNo] withFilterBy:filterString withQuery:searchBar.text withFlag:@"" withLastArticleId:[curatedNews valueForKey:@"articleId"]];
                }
            } else {
                if([folderId isEqualToNumber:[NSNumber numberWithInt:0]] && [newsLetterId isEqualToNumber:[NSNumber numberWithInt:0]]) {
                    if([category isEqualToNumber:[NSNumber numberWithInt:-2]]) {
...
-(void)requestChange:(id)sender {
    ...
    if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPhone) {
        ...
        formSheet.willPresentCompletionHandler = ^(UIViewController *presentedFSViewController) {
            ...
            navController.topViewController.title = @"Request Change";
        };
        formSheet.transitionStyle = MZFormSheetTransitionStyleCustom;
